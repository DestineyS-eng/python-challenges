def valid_phone_number(phone_number):
    ...
    if len(phone)==2:
        #if anywhere outside mandatory punctuation is a integer
        if phone[0][1:4].isdigit() and phone[1][0:3].isdigit() and phone[1][4:].isdigit():
            #if the length of the first element is 5  and brackets in the correct position 
            if len(phone[0])==5 and phone_number[0]=="(" and phone_number[4]==")":
                #if there is a space in this area only 
                if phone_number[5]==" ":
                    #if the length of the second element is 8 and the hyphen is in position 9
